...
* Created the primary Click command routing standard arguments (`STATE`) and options (`--keyword`, `--organization`, `--geographic`, `--abstract`, `--title`, and `--author`).
* Enabled multi-value parameters (e.g. `--keyword sediment --keyword sand`, `--title greenhouse`, `--author Raymond`).
